...
DYN_RANGE = (-0.2, 0.2)  # PLOTTING ONLY
FWHM_RANGE = (-0.25, 0.25)  # coarse cut to remove extreme outliers
...
if PLOT > 0:
    fig, ax = plt.subplots(ncols=4, figsize=(25, 7), gridspec_kw={
    ...
    h = ax[1].imshow(Hstd.T, cmap='Greens', extent=(
        0, xedges[-1], 0, yedges[-1]))
    plt.colorbar(h, ax=ax[1], orientation="horizontal")
    # ax[1].scatter(mmx, mmy, marker='x', c='k', alpha=0.5)

    ax[2].imshow(digit_H.T - med_fwhm, cmap='coolwarm', vmin=DYN_RANGE[0],
                 vmax=DYN_RANGE[1], extent=(0, xedges[-1], 0, yedges[-1]))

    ax[2].text(0.01, 1.01, s=f'{FILENAME}', transform=ax[0].transAxes)
    ax[3].hist(fwhm_image, bins=np.arange(med_fwhm+DYN_RANGE[0], med_fwhm +
    ...
    ax[3].hist(H[~np.isnan(H)].flatten(), bins=np.arange(med_fwhm+DYN_RANGE[0],
                                                         med_fwhm+DYN_RANGE[1]+0.01, 0.01), histtype='step', label='Spatial bins')
    ax[3].hist(digit_H[~np.isnan(H)].flatten(), histtype='step',
               bins=bin_edges, label='Digitized spatial bins')
    ax[2].text(
        0.01, 1.01, s=f'N={len(fwhm_image)} | Median(FWHM) = {med_fwhm:3.3f} px', transform=ax[2].transAxes)
